...
def decrypt(key0, key1, ct):
    ...
    r1 = key1.decrypt(ct1, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),
                           algorithm=hashes.SHA256(), label=None))
    # Attack:
    # 1st message [r1, r1 ^ message1]
    # 2nd message [r2, r2 ^ message2]
    # Then send
    # [r1, r2^message]; because the same key is used, get back r1 ^ r2 ^ message2
    # [r2, r1^message] gives back r2 ^ r1 ^ message1
    # [r3, r2^message] gives back r3 ^ r2 ^ message2
    # [r3, r1^message] gives back r3 ^ r1 ^ message1
    # r3 ^ r1 ^ r2 ^ message ^ r2 ^ message ^ r3 ^ r1 ^ message1 = message I think 
    # r0 ^ r0 ^ msg = msg
    return strxor(r0, r1)
